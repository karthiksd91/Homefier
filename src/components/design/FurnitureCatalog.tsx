    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex gap-1 flex-wrap mb-3">
        <button
          ...
          className={`px-2 py-1 rounded-md text-xs transition-all ${furnitureCategory === 'all' ? 'bg-primary-600 text-white' : 'text-slate-400 hover:text-white hover:bg-white/10'}`}
        >
          All
        </button>
        {FURNITURE_CATEGORIES.map(cat => (
          <button
            ...
            className={`px-2 py-1 rounded-md text-xs transition-all ${furnitureCategory === cat ? 'bg-primary-600 text-white' : 'text-slate-400 hover:text-white hover:bg-white/10'}`}
          >
          ...
          {filtered.map(item => {
            ...
            return (
              <button
                ...
                `}
              >
                <div className="text-2xl mb-1.5 group-hover:scale-110 transition-transform">{item.thumbnail}</div>
      ...
      {pendingFurnitureId && (
        <div className="mt-3 px-3 py-2 rounded-lg bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 text-xs text-center">
          Click in the 3D scene to place
        </div>
      )}
    </div>
  )
